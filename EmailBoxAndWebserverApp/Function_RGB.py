...
class EmailKontoBox(object):
    ...
    def save_to_Database(self,Message, Sender, Zeit):
        actual_Message = Nachricht(Nachricht_text=Message, zeitstempel=float(Zeit), EmailAddresse=Sender,
                                   Showen=True, zeitstring=strftime("%H:%M  %d %b %Y  ", gmtime(int(float(Zeit)))))
        actual_Message.save();



    def update_message_Database(self):
